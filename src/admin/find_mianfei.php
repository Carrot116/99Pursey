...
if (hxl_isset_post('state'))
    $state=$_POST['state'];
if (hxl_isset_post('type'))
    $type=$_POST['type'];
if(hxl_post_strValue('type')==""){
$state=$_GET['state'];
$type=$_GET['type'];
}
if($state=="all"){
	$sql1=hxl_mysql_query("select count(*) as total from tb_info where type='$type' order by edate");
}else{
	$sql1=hxl_mysql_query("select count(*) as total from tb_info where type='$type' and checkstate=$state order by edate");
}
$minfo=hxl_mysql_fetch_array($sql1);
$total=$minfo['total'];
$pagesize=10;
if($total<=$pagesize){
    $pagecount=1;
...
//}
if($state=="all"){
	$sql=hxl_mysql_query("select * from tb_info where type='$type' order by edate limit ".($page-1)*$pagesize.",$pagesize");
}else{
	$sql=hxl_mysql_query("select * from tb_info where type='$type' and checkstate=$state order by edate limit ".($page-1)*$pagesize.",$pagesize");
}
...
<table width="776" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
	...
	<?php
	}while($info=hxl_mysql_fetch_array($sql));
	?>
  <tr bgcolor="#FFFFDD">
