...
<?php
$date1  = date("Y-m-d");
$sgsql  = hxl_mysql_query("select * from tb_leaguerinfo  where type='家教信息' and showday>='$date1' and checkstate=1 ");
$sginfo = hxl_mysql_fetch_array($sgsql);
if ($sginfo) {
	do {
		?>
		<table width="540" border="0" cellspacing="0" cellpadding="0">
		            <tr>
		              <td height="26"><span class="style1">『家教信息』</span><span class="style8">&nbsp;
		<?php echo $sginfo['title'];
		?>&nbsp;
		</span><span class="style6">&nbsp;
		<?php echo $sginfo['edate'];
		?></span></td>
		            </tr>
		            <tr>
		              <td height="26">&nbsp;
		...
		&nbsp;
		<span class="style5">&nbsp;
		<?php echo $sginfo['content'];
		?></span></td>
		            </tr>
		            <tr>
		              <td height="26">&nbsp;
		<span class="style8">联系人：<?php echo $sginfo['linkman'];
		?>&nbsp;
		&nbsp;
		&nbsp;
		...
		<?php
	} while ($sginfo = hxl_mysql_fetch_array($sgsql));
} else {
	?>
	<table width="540" border="0" cellspacing="0" cellpadding="0">
	            <tr>
...
<br>
<?php
$sql      = hxl_mysql_query("select count(*) as total from tb_info where type='家教信息' and checkstate=1");
...
$page = hxl_get_intValue('page',1);
//if (($_GET['page']) == "") {
//	$page = 1;
//} else {
//	$page = intval($_GET['page']);
//}
$gsql  = hxl_mysql_query("select * from tb_info where type='家教信息' and checkstate=1 order by edate desc limit ".($page-1)*$pagesize.",$pagesize");
$ginfo = hxl_mysql_fetch_array($gsql);
if ($ginfo) {
	do {
		?>
		<table width="540" border="0" cellspacing="0" cellpadding="0">
		...
		&nbsp;
		<span class="style5">&nbsp;
		<?php echo $ginfo['content'];
		?></span></td>
		            </tr>
		            <tr>
		              <td height="26">&nbsp;
		<span class="style8">联系人：<?php echo $ginfo['linkman'];
		?>&nbsp;
		&nbsp;
		&nbsp;
		联系电话：<?php echo $ginfo['tel'];
		?></span></td>
		            </tr>
		            <tr>
		              <td height="3" background="Images/line1.gif"></td>
		...
		<?php
	} while ($ginfo = hxl_mysql_fetch_array($gsql));
	?>
	...
	if ($pagecount <= 4) {
		for ($i = 1; $i <= $pagecount; $i++) {
			?>
			      <a href="teaching.php?page=<?php echo $i;?>"><?php echo $i;
			?></a>
			<?php
		}
	} else {
